...
Core Parameters: Temperature & Top-P sliders. yes
Throttle: Concurrent Threads slider. - yes
Burn Rate: Batch vs Ultra toggle.
 -yes 
Collateral Overwrite (Skip vs Replace): - yes
Auto-Retry Threshold: stop and leave it alone run throught he batch and put it in the report and leave it in the undone section
Output Restraint (Max Tokens): yes we need to know the max allowed for each model per the docs. 
Structured Format Lock:  - yes 
Context Auto-Truncation:  who said 120 k is max? we cant make shit up. 



what about the visual strike status dashboard with live real time feeds to monitor the api siuation



**Rev Limiter / Governor** panel. 
real-time 
Tokens-Per-Second (TPS) 
Requests-Per-Minute (RPM) gauges, 

I've logged these exact specs into the tracker. I am tearing into the `App.tsx` frontend React DOM right now to blow away the 3 columns and rebuild this massive 4-Pane LIM interface.
